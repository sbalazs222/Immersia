...
export const mailService = {
  ...
  async resendConfirmAddressToken(token) {
    try {
      ...
      const [exists] = await pool.query('SELECT * FROM email_codes WHERE token_hash = ? AND expires_at < NOW() AND type = "confirm";', [tokenHashOld]);

      if (exists.length < 1) throw new ApiError(404, 'INVALID_TOKEN');

      const [email] = await pool.query(
        'SELECT AES_DECRYPT(email, ?) as email FROM users WHERE id = ?',
        [env.DB_ENCRYPT_SECRET, exists[0].user_id]);
